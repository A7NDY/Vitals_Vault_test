...
export default function Patients() {
  return (
    <MainLayout>
      <h1 className="mb-6 text-3xl font-semibold tracking-tight text-slate-900">Patient Management</h1>

      <section className="rounded-xl border bg-white shadow-sm">
        <div className="border-b p-4 text-sm font-medium text-slate-700">Pending Patient Requests</div>
          ...
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50/60">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Condition</th>
                <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Insurance/ID Proof</th>
            ...
            <tbody className="divide-y divide-slate-200 bg-white">
              {pending.map((p) => (
                <tr key={p.email}>
                  ...
                  <td className="whitespace-nowrap px-6 py-4 text-sm text-slate-600">{p.condition}</td>
                  <td className="whitespace-nowrap px-6 py-4 text-sm">
                    <button className="text-slate-700 underline hover:text-slate-900">Download</button>
                  </td>
                  <td className="whitespace-nowrap px-6 py-4 text-sm">
      ...
      <section className="mt-10">
        <div className="mb-3 text-sm font-medium text-slate-700">Approved Patients</div>
        ...
        <div className="mb-4 flex items-center gap-3">
          ...
          <div className="hidden gap-2 md:flex">
            <button className="rounded-md border bg-white px-3 py-2 text-xs text-slate-700 shadow-sm hover:bg-slate-50">Condition ▾</button>
            ...
            <table className="min-w-full divide-y divide-slate-200">
              <thead className="bg-slate-50/60">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Name</th>
                  <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Condition</th>
                  <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Total Visits</th>
              ...
              <tbody className="divide-y divide-slate-200 bg-white">
                {approved.map((p) => (
                  <tr key={p.name}>
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-slate-700">{p.name}</td>
                    <td className="px-6 py-4 text-sm text-sky-600">{p.condition}</td>
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-slate-600">{p.visits}</td>
                    <td className="whitespace-nowrap px-6 py-4 text-sm">
                      <span
