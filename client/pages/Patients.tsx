...
const PATIENTS: Patient[] = [
  ...
  },
  {
    ...
    email: "isabella.coleman@email.com",
    assigned: "Dr. David Brown",
    status: "Active",
  ...
  {
    name: "Lucas Sullivan",
    email: "lucas.sullivan@email.com",
    assigned: "Dr. David Brown",
    status: "Inactive",
...
export default function Patients() {
  ...
  const doctors = useMemo(
    () => ["All", ...Array.from(new Set(PATIENTS.map((p) => p.assigned)))],
    [],
  );
          ...
          <div className="flex w-full items-center gap-3">
            <div className="relative w-full max-w-3xl">
              ...
              <input
                ...
                className="h-10 w-full rounded-md border border-slate-200 bg-white pl-10 pr-3 text-sm text-slate-700 placeholder:text-slate-400 focus:border-slate-300 focus:outline-none"
              />
            </div>
          </div>
              ...
              {filtered.map((p) => (
                <tr key={p.email}>
                  ...
                  <td className="px-6 py-4 text-sm">
                    <span
                      className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ${p.status === "Active" ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-700"}`}
                    >
                      {p.status}
                    </span>
                  </td>
