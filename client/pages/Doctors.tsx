...
export default function Doctors() {
  ...
  if (user && user.role === "Doctor") {
    ...
    const counts = useMemo(
      () => ({ total: 120, normal: 80, intermediate: 30, critical: 10 }),
      [],
    );
        ...
        <div>
              ...
              {vitalsOverview.map((v) => (
                ...
                >
                  ...
                  <div className="mt-1 text-sm text-slate-400">
                    {v.note} •{" "}
                    <span
                      className={`${v.change.startsWith("+") ? "text-emerald-600" : "text-rose-600"}`}
                    >
                      {v.change}
                    </span>
                  </div>
                  <svg
                    className="mt-4 h-24 w-full text-slate-300"
                    viewBox="0 0 120 32"
          ...
          <section>
            ...
            <div className="flex flex-wrap gap-3">
              <button className="rounded-md bg-sky-600 px-4 py-2 text-sm font-medium text-white">
                Add Patient
              </button>
